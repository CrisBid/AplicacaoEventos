...
class UserController {
    ...
    private $jwtAlgorithm = 'HS256'; //algoritmo de assinatura
    ...
    public function handle($method, $data, $route) {
        ...
        switch ($route) {
            ...
            case 'users/login':
                ...
                break;
            case 'users/delete/' . $data['id']:
                if ($method == 'DELETE') {
                    $this->handleDeleteUser($user, $data['id']);
                }
                break;
            case 'users/update':
                if ($method == 'PUT') {
    ...
    private function handleDeleteUser($user, $userId) {
